...
insert into city values (6,'Rotterdam','NLD','Zuid-Holland',13456);
...
where order_date = customer_preef_delivery_date)/count(*)*100,2) 
...
select count(team_id) from employees;
select employees_id, count(team_id) over (partition by team_id) as team_size from employees order by employees_id;
...
(7,'peru'),(5,'china'),(8,'moracco'),(9,'spain');
